...
interface SimpleLiveStreamViewerProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function SimpleLiveStreamViewer({
  isOpen,
  onClose,
  stream,
  isHost = false,
}: SimpleLiveStreamViewerProps) {
  const { user } = useAuth();
  ...
  const initializeViewerStream = async () => {
    try {
      ...
      // For now, we'll show a placeholder for viewers
      // In a real implementation, this would connect to the host's stream
      if (videoRef.current) {
