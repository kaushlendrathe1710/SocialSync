...
interface WebRTCLiveStreamViewerProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function WebRTCLiveStreamViewer({
  isOpen,
  onClose,
  stream,
  isHost = false,
}: WebRTCLiveStreamViewerProps) {
  const { user } = useAuth();
      ...
      ws.onmessage = (event) => {
        try {
          ...
          switch (data.type) {
            ...
            case "create-offer":
              if (isHost && peerConnection && localStream) {
                createOfferForViewer(data.fromUserId);
              }
              break;
            case "offer":
  ...
  const initializeHostStream = async () => {
    try {
      ...
      // Handle incoming streams (for when viewers connect)
      pc.ontrack = (event) => {
      ...
      setConnectionState("Ready to stream");
      setIsLoading(false);
    } catch (error) {
  ...
  // Create offer for viewer (host side)
  const createOfferForViewer = async (targetUserId: number) => {
